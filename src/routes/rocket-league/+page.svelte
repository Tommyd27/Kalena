<script lang="ts">
	...
	let playersOutput  : Array<Object>  = [];
	let rows = ["Name", "MMR"];
...
<button on:click={startListening}
	>PRESS ME PRESS ME PRESSES ME NOM NUM NUM</button
>
...
<div class="tableGrid" style="--playerNumbers: {playerNumbers}">
	...
	{#each statsToTrack as row, i}
		<h1 style="grid-row-start: {2 * i + 4}; grid-row-end: {2 * i + 4}">
			{row}
		</h1>
		<h1
			style="grid-row-start: {2 * i + 5}; grid-row-end: {2 * i +
	...
	{#each players as player, i}
		<h1
			style="grid-row-start: 1; grid-row-end:1; grid-column-start:{i +
				2}; grid-column-end:{i + 2}"
		>
			{player.name}
		</h1>
		<h1
			style="grid-row-start: 2; grid-row-end:2; grid-column-start:{i +
				2}; grid-column-end:{i + 2}"
		>
		...
		{#each statsToTrack as row, j}
			<input
				...
				style="grid-row: 1; grid-row-start:{2 * j +
					4}; grid-row-finish:{2 * j + 4}; grid-column-start:{i +
					2}; grid-column-end:{i + 2}"
			/>
			<input
				type="range"
				...
				style="grid-row: 2;grid-row-start:{2 * j +
					5}; grid-row-finish:{2 * j + 5}; grid-column-start:{i +
					2}; grid-column-end:{i + 2}"
			/>
		{/each}
	{/each}
